...
export default function ManageLockAccessScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (pendingInvites.length > 0) {
        const newPending = pendingInvites.filter(
          pendingEmail =>
            !fetchedUsers.some(user => user.email === pendingEmail),
        );
        if (newPending.length < pendingInvites.length) {
          Toast.show({
            type: 'info',
            text1: 'User joined!',
            text2: 'The user list has been updated.',
          });
        }
      ...
      setLockGroup(group);
      if (group) {
        const initialUserIds = new Set(group.userIds || []);
        setInitialUsers(initialUserIds);
        const selectedUserIds = new Set(initialUserIds);
        const owner = fetchedUsers.find(u => u.role === 'owner');
        if (owner) {
          selectedUserIds.add(owner.id);
        }
  ...
  }, [token, role, ctxLockId, activeWorkspace?.workspace_id, pendingInvites]);
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      const usersToRemove = [...initialUsers].filter(
        u => !selectedUsers.has(u),
      );
      ...
      const rebuildRes = await rebuildAcl(
        token,
        activeWorkspace.workspace_id,
        Number(ctxLockId),
      );
      ...
      setUpdateStatus('Updating User Access...');
      const data = await fetchLatestAcl(
        token,
        activeWorkspace.workspace_id,
        Number(ctxLockId),
      );
      ...
      if (
        e.message !== 'missing-userpubs' &&
        e.message !== 'missing-ownership'
      ) {
        Toast.show({
          type: 'error',
          text1: 'Update Failed',
  ...
  const handleFindUser = async () => {
    ...
    try {
      ...
      if (res.ok && res.user) {
        ...
        setAllUsers(prevUsers => [...prevUsers, res.user]);
  ...
  const handleInviteUser = async () => {
    ...
    try {
      ...
      Toast.show({ type: 'success', text1: 'successfully invited user' });
      setPendingInvites(prev => [...prev, addUserEmail]);
      setIsModalVisible(false);
      setAddUserEmail('');
      setInviteRole(null);
    } catch (error) {
  ...
  React.useEffect(
    () =>
      nav.addListener('beforeRemove', e => {
        ...
        Alert.alert(
          'Discard changes?',
          `You have ${numberOfChanges} unsaved changes. Are you sure you want to discard them and leave the screen?`,
          [
  ...
  return (
    <View style={s.c}>
      <Text style={s.t}>
        Manage Access for {ctxLockName || `Lock #${ctxLockId}`}
      </Text>
      ...
      {ctxLockId ? (
        <TouchableOpacity
          style={[
            s.btn,
            {
              backgroundColor: numberOfChanges > 0 ? '#7B1FA2' : 'gray',
              marginTop: 12,
            },
          ]}
        ...
        >
          {busy ? (
            ...
            <Text style={s.bt}>
              {numberOfChanges > 0
                ? `Update (${numberOfChanges}) Changes`
                : 'No changes to sync'}
            </Text>
          )}
        </TouchableOpacity>
      ) : null}
      <TouchableOpacity style={s.fab} onPress={handleAddUserPress}>
            ...
            {modalView === 'addUser' ? (
              ...
              <>
                <Text style={s.modalTitle}>
                  User is not in your workspace, send an invite
                </Text>
                <DropDownPicker
        ...
        <View style={s.modalContainer}>
          <View style={s.modalContent}>
            ...
            <Text style={s.modalText}>
              You have {numberOfChanges} change{numberOfChanges > 1 ? 's' : ''}{' '}
              pending. To sync, you must be near the lock.
            </Text>
            <Text style={s.modalText}>
              Step 1: Stand next to "{ctxLockName || `Lock #${ctxLockId}`}".
            </Text>
            <Text style={s.modalText}>
              Step 2: Tap the button below to connect.
            </Text>
            <TouchableOpacity
            ...
            >
              {isSyncing ? (
                <View style={s.busyContainer}>
                  <ActivityIndicator color="#fff" size="small" />
                  <Text style={s.btBusy}>
                    Syncing... Do not close the app or exit screen
                  </Text>
                </View>
              ) : (
