...
export default function ClaimLockScreen() {
  ...
  const doClaim = async () => {
    ...
    try {
      ...
      device = await scanAndConnectForLockId(Number(lockIdToClaim));
      await sendOwnershipSet(device, {
        lockId: Number(lockIdToClaim),
        adminPubB64: res.adminPubB64.trim(),
        claimCode: claimCodeToClaim.trim(),
      });

      await patchLock(token, workspaceId, Number(lockIdToClaim), { setupComplete: true });
      await clearClaimContext(Number(lockIdToClaim));
      await refreshUser();
    ...
    <ScrollView style={s.c}>
      ...
      >
        <View style={s.centeredView}>
          <View style={s.modalView}>
            <Text style={s.modalText}>Adding New Lock</Text>
            <Text style={s.modalGuide}>
              Please make sure the lock is fully setup and in working condition
            </Text>
            <Text style={s.modalGuide}>Please stand near the lock.</Text>
            <Text style={s.modalGuide}>
              Don't turn off the app or exit the screen.
            </Text>
          </View>
        </View>
      </Modal>
      ...
      {claimMode === null && (
        <View style={s.choiceContainer}>
          ...
          >
            <Text style={s.bt}>Scan QR Code</Text>
          </TouchableOpacity>
