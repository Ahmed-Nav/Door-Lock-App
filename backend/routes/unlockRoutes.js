...
router.post("/token", requireAuthJSON, async (req, res) => {
  try {
    const base64 = process.env.LOCK_SECRET_BASE64;
    if (!base64) return res.status(500).json({ error: "LOCK_SECRET_BASE64 not set" });
    const Kd = Buffer.from(base64, "base64"); // 32 bytes

    const V = Buffer.from([0x01]);                                  // 1
    const ts = Buffer.alloc(4); ts.writeUInt32BE(Math.floor(Date.now()/1000), 0); // 4
    const nonce = crypto.randomBytes(4);                             // 4
    const data = Buffer.concat([V, ts, nonce]);                      // 9 bytes

    const mac = crypto.createHmac("sha256", Kd).update(data).digest().subarray(0, 8); // 8
